function love.load()
    ...
    FontDifficulties = love.graphics.newFont("Fonts/pixel-emulator-font/PixelEmulator-xq08.ttf", 30)
    ...
    difficultyEasy.Name = "Easy"
    difficultyMedium.Name = "Medium"
    difficultyHard.Name = "Hard"

    difficultyEasy.spawnSpeed = 1.5
    difficultyMedium.spawnSpeed = 1
    difficultyHard.spawnSpeed = 0.4

    -- difficultyEasy.IncreaseSpeed = 0.95
    -- difficultyMedium.IncreaseSpeed = 0.9
    -- difficultyHard.IncreaseSpeed = 0.8

    difficultyEasy.x = love.graphics.getWidth() - (love.graphics.getWidth() / 5 * 4)
    difficultyMedium.x = love.graphics.getWidth() - (love.graphics.getWidth() / 2)
    difficultyHard.x = love.graphics.getWidth() - (love.graphics.getWidth() / 5)
    
    difficultyEasy.y = love.graphics.getHeight() - (love.graphics.getHeight() / 3)
    difficultyMedium.y = love.graphics.getHeight() - (love.graphics.getHeight() / 3)
    difficultyHard.y = love.graphics.getHeight() - (love.graphics.getHeight() / 3)

    difficultyEasy.zombie = spawnSpecificZombie(difficultyEasy.x, difficultyEasy.y)
    difficultyMedium.zombie = spawnSpecificZombie(difficultyMedium.x, difficultyMedium.y)
    difficultyHard.zombie = spawnSpecificZombie(difficultyHard.x, difficultyHard.y)
...
function love.update(dt)
    ...
    if gamestate == 1 then
        for i,d in ipairs(difficulties) do
            for g,b in ipairs(bullets) do
                if distanceBetween(d.x, d.y, b.x, b.y) < 20 then
...
function love.draw()
    ...
    if gamestate == 1 then
        ...
        love.graphics.setFont(FontDifficulties)

        love.graphics.printf("Select your difficulty!", 0, 50, love.graphics.getWidth(), "center")

        love.graphics.printf("Left click to shoot, right click for wave bullets", 0, 100, love.graphics.getWidth(), "center")


        for i,d in ipairs(difficulties) do
            love.graphics.draw(sprites.zombie, d.x, d.y, zombiePlayerAngle(d), nil, nil, sprites.zombie:getWidth()/2, sprites.zombie:getHeight()/2)
            local textWidth = love.graphics.getFont():getWidth(d.Name)
            local centerX = d.x - textWidth / 2
            love.graphics.printf(d.Name, centerX, d.y + 40, love.graphics.getWidth())
        end
    end
...
function newGame()
    gamestate = 2
    maxTime = SelectedDifficulty.spawnSpeed
    timer = maxTime
    score = 0
    bulletSpinCharges = 0
    bulletSpinChargeMaxTime = 4
...
function spawnSpecificZombie(x, y)
    local zombie = {}
    zombie.x = x
    zombie.y = y
    --zombie.speed = math.random(zombieSpeedRange.MinSpeed, zombieSpeedRange.MaxSpeed)
